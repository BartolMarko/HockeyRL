...
MODEL_PATH = Path(__file__).resolve().parent / "results" / "baseline_dqn" / "model_step_500038.pt"
GIF_SAVE_PATH = Path(__file__).resolve().parent / "results" / "baseline_dqn" / "evaluation.gif"
...
def evaluate(env, agent, opponent, num_episodes, step, render=False, save=None):
	...
	for i in range(num_episodes):
		...
		while not done:
			...
			action, _ = agent.plan(obs, eval_mode=True, step=step)
