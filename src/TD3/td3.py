...
from src.TD3.replay_buffer import (
    ReplayBuffer,
...
class TD3(NamedAgent):
    def __init__(self, config: dict):
        ...
        self._config = {
            ...
            "buffer_size": int(1e6),
            "batch_size": 128,
            "actor": {"hidden_sizes": [256, 256], "learning_rate": 1e-4},
            "critic": {"hidden_sizes": [256, 256], "learning_rate": 1e-3},
            "polyak": 0.995,
            "target_noise": 0.2,
            "noise_clip": 0.5,
            "policy_delay": 2,
        ...
        self.output_activation = lambda x: (
            self.low + (self.high - self.low) * (nn.Tanh()(x) + 1.0) / 2
        )
        ...
        self.policy_optimizer = torch.optim.Adam(
            self.model.actor.parameters(), lr=self._config["actor"]["learning_rate"]
        )

        self.q_params = list(
            itertools.chain(
                self.model.critic1.parameters(), self.model.critic2.parameters()
            )
        )
        self.critic_optimizer = torch.optim.Adam(
            self.q_params, lr=self._config["critic"]["learning_rate"]
        )
        ...
        if self.pr_replay:
            self.buffer = PERNumpy(
                self._obs_dim,
                self._action_n,
                self._config["buffer_size"],
                alpha=self._config["pr_alpha"],
                beta=self._config["pr_beta"],
            )
        else:
            self.buffer = ReplayBuffer(
                self._obs_dim, self._action_n, self._config["buffer_size"]
            )
    ...
    def compute_q_loss(self, data):
        ...
        with torch.no_grad():
            ...
            eps = (
                torch.randn_like(pi_targ, device=device) * self._config["target_noise"]
            )
            eps = torch.clamp(
                eps, -self._config["noise_clip"], self._config["noise_clip"]
            )
        ...
        td_error1 = q1 - target
        td_error2 = q2 - target
        td_errors = torch.abs(td_error1 + td_error2) * 0.5
    ...
    def update(self, t):
        ...
        if self._config.get("grad_clip_norm") is not None:
            nn.utils.clip_grad_norm_(
                self.q_params, max_norm=self._config["grad_clip_norm"]
            )
        ...
        if t % self._config["policy_delay"] == 0:
            ...
            if self._config.get("grad_clip_norm") is not None:
                nn.utils.clip_grad_norm_(
                    ...
                    max_norm=self._config["grad_clip_norm"],
                )
    ...
    def QValues(self, obs: np.ndarray, action: np.ndarray) -> list[float]:
        ...
        with torch.no_grad():
            return self.model.q1(obs, action).item(), self.model.q2(obs, action).item()
