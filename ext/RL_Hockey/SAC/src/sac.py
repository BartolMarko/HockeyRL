...
from ext.RL_Hockey.SAC.src.noise import *
from ext.RL_Hockey.SAC.src.memory import (
    ReplayMemory,
    PrioritizedExperienceReplay,
    EREPrioritizedExperienceReplay,
)
from ext.RL_Hockey.SAC.src.policies import Actor, Critic
...
class SACAgent:
    def __init__(self, observation_space, action_space, **userconfig):
        ...
        if self.control_half:
            ...
            new_action_space = gym.spaces.Box(
                new_low, new_high, dtype=action_space.dtype
            )
        else:
        ...
        if self._config["use_per"]:
            if self._config["use_ere"]:
                self.buffer = EREPrioritizedExperienceReplay(
                    ...
                    beta_increment=self._config["beta_increment"],
                    epsilon=self._config["epsilon"],
    ...
    def train(self, iter_fit=32):  # gradient update steps
        ...
        for k in range(iter_fit):
            ...
            if isinstance(self.buffer, PrioritizedExperienceReplay):
                ...
                state = torch.FloatTensor(experiences[0]).to(
                    device
                )  # Already stacked states
                action = torch.FloatTensor(experiences[1]).to(
                    device
                )  # Already stacked actions
                reward = torch.FloatTensor(experiences[2][:, None]).to(
                    device
                )  # Add dimension for rewards
                next_state = torch.FloatTensor(experiences[3]).to(
                    device
                )  # Already stacked next_states
                done = torch.FloatTensor(experiences[4][:, None]).to(
                    device
                )  # Add dimension for dones
    ...
    def restore_full_state(self, checkpoint):
        ...
        if actor_opt_sd is not None:
            ...
            print(
                "[SACAgent] 'actor_optimizer_state_dict' missing. Skipping actor optimizer load."
            )
        ...
        if critic1_opt_sd is not None:
            ...
            print(
                "[SACAgent] 'critic1_optimizer_state_dict' missing. Skipping critic1 optimizer load."
            )
        ...
        if critic2_opt_sd is not None:
            ...
            print(
                "[SACAgent] 'critic2_optimizer_state_dict' missing. Skipping critic2 optimizer load."
            )
        ...
        if self._config.get("learn_alpha", False):
            ...
            if alpha_opt_sd is not None:
                ...
                print(
                    "[SACAgent] 'alpha_optimizer_state_dict' missing. Skipping alpha optimizer load."
                )
        else:
