...
\begin{enumerate}
\item
We, first, train the agent against the environment WeakBot and StrongBot and a custom PuckFollowBot, which follows puck vertically and hits it on contact. When the agent reaches a $85\%$ win-rate against these opponents, we proceed to the next stage.
\item
...
\end{enumerate}
These experiment results are omitted for brevity, but can be found on the wandb project and GitHub repository \cite{bartolGithubRepo}.
